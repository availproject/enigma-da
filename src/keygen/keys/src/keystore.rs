// //TODO change this to a  our scheme implemenatation using the keyshare of ECIES_threshold
use log::{debug, error};
...
use std::sync::Arc;
// add new_scheme into the theta_proto
...
//change implememtation of keys for our scheme
use super::KeyStoreError;
use super::keys::{PrivateKeyShare, PublicKey, key2id};
...
impl KeyStore {
    ...
    pub fn insert_private_key(&mut self, key: PrivateKeyShare) -> Result<String, KeyStoreError> {
        let key_id = key2id(&key.get_public_key().get_pk());

        let app_id = key.get_app_id();

        if key_id.ne(app_id) {
            error!("private Key does not match id");
    ...
    pub fn insert_public_key(&mut self, key: PublicKey) -> Result<String, KeyStoreError> {
        let key_id = key2id(&key.get_pk());

        if key_id.ne(&key.get_app_id()) {
            error!("public Key does not match id");
            return Err(KeyStoreError::IdMismatch);
        }
    ...
    pub fn get_key_by_scheme(
        ...
        let matching_key_entries: Vec<(&String, &KeyEntry)> = self
            .key_entries
            .iter()
            .filter(|&entry| entry.1.pk.get_scheme() == scheme)
            .collect();
            ...
            _ => {
                ...
                match default_key_entries.len() {
                    0 => {
                        ...
                        Err(String::from(
                            "Could not find a default key for this scheme. Please specify a key id.",
                        ))
                    }
                    ...
                    _ => {
                        ...
                        Err(String::from(
                            "Could not select a default key for this scheme. Please specify a key id.",
                        ))
                    }
                }
            }
